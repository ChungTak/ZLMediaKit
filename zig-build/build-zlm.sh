...
for arg in "$@"; do
  case $arg in
    ...
    --help)
      ...
      echo "  aarch64-windows-gnu   - ARM64 Windows (MinGW)"
...
OPENSSL_INSTALL_DIR="$SCRIPT_DIR/openssl-install-${TARGET}"
if [ ! -d "$OPENSSL_INSTALL_DIR" ]; then
    ...
    # 执行构建OpenSSL脚本
    cd "$SCRIPT_DIR"
    bash "$SCRIPT_DIR/build-openssl.sh" "--target=$TARGET"
...
if [ "$ENABLE_WEBRTC" = true ]; then
    LIBSRTP_INSTALL_DIR="$SCRIPT_DIR/libsrtp-install-${TARGET}"
    if [ ! -d "$LIBSRTP_INSTALL_DIR" ]; then
        ...
        # 执行构建libsrtp脚本
        cd "$SCRIPT_DIR"
        bash "$SCRIPT_DIR/build-libsrtp.sh" "--target=$TARGET"
...
# 创建临时工具链文件
TOOLCHAIN_FILE="zig-toolchain-${TARGET}.cmake"
...
CMAKE_ARGS=(
    ...
    -DOPENSSL_ROOT_DIR="$SCRIPT_DIR/openssl-install-${TARGET}/"
    -DOPENSSL_LIBRARIES="$SCRIPT_DIR/openssl-install-${TARGET}/lib/"
...
if [ "$ENABLE_WEBRTC" = true ]; then
    ...
    CMAKE_ARGS+=(
        -DENABLE_WEBRTC=ON
        -DSRTP_INCLUDE_DIRS="$SCRIPT_DIR/libsrtp-install-${TARGET}/include/"
        -DSRTP_LIBRARIES="$SCRIPT_DIR/libsrtp-install-${TARGET}/lib/libsrtp2.a"
    )
else
    echo "禁用WebRTC支持..."
    CMAKE_ARGS+=(
